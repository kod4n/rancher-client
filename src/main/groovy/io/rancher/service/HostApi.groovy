...
interface HostApi {
  ...
  @GET('v2-beta/hosts/{id}')
  Call<Host> findById(@Path('id') String id)

  @POST('v2-beta/hosts/{id}?action=evacuate')
  Call<Host> evacuate(@Path('id') String id)

  @POST('v2-beta/hosts/{id}?action=provision')
  Call<Host> provision(@Path('id') String id)

  @POST('v2-beta/hosts/{id}?action=dockersocket')
  Call<HostAccess> dockersocket(@Path('id') String id)

  @POST('v2-beta/hosts/{id}?action=activate')
  Call<Host> activate(@Path('id') String id)

  @POST('v2-beta/hosts/{id}?action=purge')
  Call<Host> purge(@Path('id') String id)

  @POST('v2-beta/hosts/{id}?action=error')
  Call<Host> error(@Path('id') String id)

  @POST('v2-beta/hosts/{id}?action=remove')
  Call<Host> remove(@Path('id') String id)

  @POST('v2-beta/hosts/{id}?action=deactivate')
